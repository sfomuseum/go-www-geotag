...
debug:
	@make bake
	go run -mod vendor cmd/server/main.go -nextzen-apikey $(APIKEY) -crumb-uri debug -enable-placeholder -placeholder-endpoint $(SEARCH) -enable-oembed -oembed-endpoints 'https://millsfield.sfomuseum.org/oembed/?url={url}&format=json' -enable-writer -enable-proxy-tiles -proxy-tiles-cache-uri fs:///tmp/tiles -enable-map-layers
